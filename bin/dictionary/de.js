...
global.Dictionary.dateFormat('de', { day: 0, month: 1, year: 2, sep: '. ', })
...
global.Dictionary.add('de', {
  config: {
    ...
    methods: {
      lang: {
        name: 'sprache',
        resume: 'Ändern Sie die Bot-Sprache',
        doc: _Message => {
          _Message.set(
      ...
      prefix: {
        name: 'prafix',
        resume: 'Ändert das Präfix des Bots',
        doc: (_Message, { prefix, }) => {
          _Message.set(
            'Präfix ist der Text, der vor einem Befehl eingegeben wird, damit der Bot weiß, dass ' +
              `er für ihn ist. Standardmäßig verwenden wir das Präfix \`${prefix}\`. Derzeit ist ` +
              `dieser Server so konfiguriert, dass er das Präfix \`${prefix}\` verwendet. Sie ` +
              'können es jedoch mit diesem Befehl ändern\n\n' +
              '***Beispiel:***\n\n' +
              `\`${prefix}einrichten prafix dont:\``
          )
        },
      },
    },
    messages: {
      ...
      UPDATED_PREFIX: 'Präfix erfolgreich aktualisiert',
      UPDATE_PREFIX_ERROR: 'Beim Ändern des Präfixes ist ein Problem aufgetreten',
    },
  },
  experience: {
    methods: {
