...
global.Dictionary.dateFormat('it', { day: 0, month: 1, year: 2, sep: '/', })

// Adiciona sessão para portugês do brasil ao dicionário
global.Dictionary.add('it', {
  config: {
    name: 'impostare',
    resume: 'Configura il robot sul server (solo proprietario del server)',
    methods: {
      lang: {
        name: 'lingua',
      ...
      prefix: {
        name: 'prefisso',
        resume: 'Cambia il prefisso del Bot',
        doc: (_Message, { prefix, }) => {
          _Message.set(
            'Il prefisso è il testo inserito prima di un comando in modo che il Bot sappia che è ' +
              `per lui. Per impostazione predefinita usiamo il prefisso \`${prefix}\` e ` +
              `attualmente questo server è configurato per usare il prefisso \`${prefix}\`, ma ` +
              'puoi cambiarlo con questo comando\n\n' +
              '***Esempio:***\n\n' +
              `\`${prefix}impostare prefisso dont:\``
          )
        },
      },
    },
    messages: {
      ...
      UPDATED_PREFIX: 'Prefisso aggiornato correttamente',
      UPDATE_PREFIX_ERROR: 'Si è verificato un problema durante la modifica del prefisso',
    },
  },
  experience: {
    methods: {
