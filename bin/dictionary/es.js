...
global.Dictionary.dateFormat('es', { day: 0, month: 1, year: 2, sep: '/', })

// Adiciona sessão para portugês do brasil ao dicionário
global.Dictionary.add('es', {
  config: {
    name: 'config',
    resume: 'Configura el robot en el servidor (solo el propietario del servidor)',
    methods: {
      lang: {
        name: 'idioma',
      ...
      prefix: {
        name: 'prefijo',
        resume: 'Cambia el prefijo del bot',
        doc: (_Message, { prefix, }) => {
          _Message.set(
            'Prefijo es el texto ingresado antes de un comando para que el Bot sepa que es para ' +
              `él. De manera predeterminada, usamos el prefijo \`${prefix}\` y actualmente ` +
              `este servidor está configurado para usar el prefijo \`${prefix}\`, pero puede ` +
              'cambiarlo con este comando\n\n' +
              '***Ejemplo:***\n\n' +
              `\`${prefix}config prefijo dont:\``
          )
        },
      },
    },
    messages: {
      ...
      UPDATE_LANGUAGE_ERROR: 'Se produjo un problema al cambiar el idioma.',
      UPDATED_PREFIX: 'Prefijo actualizado correctamente',
      UPDATE_PREFIX_ERROR: 'Hubo un problema al cambiar el prefijo',
    },
  },
  experience: {
    methods: {
