...
global.Dictionary.dateFormat('fr', { day: 0, month: 1, year: 2, sep: '/', })
...
global.Dictionary.add('fr', {
  config: {
    ...
    methods: {
      lang: {
        name: 'langue',
        resume: 'Changer la langue du bot',
        doc: _Message => {
          _Message.set(
            'Accepte les valeurs::\n\n' +
            '> 🇩🇪 `de` - Deutsche\n' +
      ...
      prefix: {
        name: 'prefixe',
        resume: 'Modifie le préfixe du Bot',
        doc: (_Message, { prefix, }) => {
          _Message.set(
            'Le préfixe est le texte saisi avant une commande pour que le Bot sache que c\'est ' +
              `pour lui. Par défaut, nous utilisons le préfixe \`${prefix}\` et actuellement ce ` +
              `serveur est configuré pour utiliser le préfixe \`${prefix}\`, mais vous pouvez le ` +
              'changer avec cette commande\n\n' +
              '***Exemple:***\n\n' +
              `\`${prefix}installer prefixe dont:\``
          )
        },
      },
    },
    messages: {
      INVALID_LANGUAGE: ({ lang, firstLangs, lastLang, }) => `OLa langue ${lang} n'est pas ` +
      ...
      UPDATED_PREFIX: 'Préfixe mis à jour avec succès',
      UPDATE_PREFIX_ERROR: 'Un problème est survenu lors de la modification du préfixe',
    },
  },
  experience: {
    methods: {
